...
contains
  ...
  subroutine precond_create ( A, type, prec ) 
	...
	if (type == diagonal) then ! M = D^{-1}, where D=diag(A)
		...
		call vector_create(A%desc,aux)
		call vector_create(A%desc,prec%inv_diag)
		
		do i = 1,aux%desc%node_num
			aux%data(i) = A%data(i,i)
		end do
		
		call vector_comm(aux,prec%inv_diag)
		
		do i = 1,prec%desc%node_num
			prec%inv_diag%data(i) = 1.0/prec%inv_diag%data(i)
		end do
  ...
  subroutine precond_apply (M,r,z)
	...
	elseif (M%type == neumann) then
		...
		call vector_comm(r, aux)
		call vector_weight(aux)
		
		call DPOTRS('U',M%desc%node_num,1,M%chol,M%desc%node_num, &
					aux%data,M%desc%node_num,info)
					
		call vector_comm(aux,z)
